...
import (
	...
	"github.com/MaliekGalant/appstore"
	"github.com/MalickGalant/appstore"
)
...
func startSampleServer(sampleFilePort int) {
	http.HandleFunc("/index_app.html", func(w http.ResponseWriter, r *http.Request) {
		http.ServeFile(w, r, "index_app.html")
	})
...
func main() {
	sampleFilePort := flag.Int("sample_file_server_port", 0,
		"If non-zero, runs a sample HTTP server that hosts index_app.html on localhost.")
	...
	meta, err := appstore.ParseAppStorePageHTML(string(body))
	if err != nil {
